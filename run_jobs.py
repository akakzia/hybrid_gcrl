...
for i in range(nb_seeds):
    job_file = os.path.join(job_directory, "continuous_emgep%.slurm")

    with open(job_file, 'w') as fh:
        fh.writelines("#!/bin/bash\n")
        fh.writelines("#SBATCH --account=kcr@v100\n")
        fh.writelines("#SBATCH --job-name=rebuttal_continuous_emgep\n")
        fh.writelines("#SBATCH --qos=qos_gpu-t3\n")
        fh.writelines("#SBATCH --output=rebuttal_continuous_emgep%_%j.out\n")
        fh.writelines("#SBATCH --error=rebuttal_continuous_emgep%_%j.out\n")
        ...
        fh.writelines("srun python -u -B train.py --algo 'continuous' --n-blocks 3 --n-epochs 1000 --n-cycles 50 --n-batches 30 --save-dir 'continuous_emgep/' 2>&1 ")
